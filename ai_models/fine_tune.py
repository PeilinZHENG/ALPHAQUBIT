...
#     # 初始化模型：不再用 ds.data.shape，而是直接用 ds.bits_per_shot
...
#         task_type=TaskType.FEATURE_EXTRACTION,  # 不用分类头
#         inference_mode=False,
#         r=8,                           # LoRA rank
#         lora_alpha=32,
#         lora_dropout=0.05,
#         # target_modules 可填模型里你想打 Adapter 的 Linear 层名
#         target_modules=["ff.0", "ff.2", "attn"]  
#     )
    
#     model = get_peft_model(model, peft_config)
#     model.print_trainable_parameters()  # 验证只有 LoRA 参数可训练

#     optimizer = torch.optim.Adam(
#         model.parameters(),  # 此时只有 LoRA adapter 在更新
#         lr=1e-3
#     )
...
#         model.train()
#         train_bar = tqdm(train_loader, desc=f"Epoch {epoch}/{args.epochs} [Train]", unit="batch")
#         for x, y in train_loader:
#             x, y = x.to(device), y.to(device)
#             optimizer.zero_grad()
 
#             logits = model(x)    # 直接把张量传进去，不要写 model(input_ids=x)
...
def train_and_save_for(folder, args, device):
    ...
    # 使用 base_model，避免 wrapper 要求 input_ids
